...
int main() {
    ...
    if (recv_len > 0) {
        ...
        if (sscanf(response, "%u %s %d %d", &task_id, op, &val1, &val2) == 4) {
            ...
            ssize_t final_len = recvfrom(sock, final_response, sizeof(final_response)-1, 0, nullptr, nullptr);
            if (final_len > 0) {
                final_response[final_len] = '\0';
                cout << "Final response: " << final_response << endl;
            } else {
                cout << "No final response received (this might be the problem!)" << endl;
            }
        }
    } else {
