...
int main(int argc, char* argv[]) {
    ...
    unordered_map<uint32_t,TaskRec> idTasks; unordered_map<ClientKey,uint32_t,ClientHash> clientToId; uint32_t nextId=1; size_t ok=0,fail=0;
    while(g_run){
        ...
        ClientKey key{ca,clen}; auto itAddr=clientToId.find(key); DBG(debugPacket(ca,clen,n,itAddr==clientToId.end()));
        if(n==(ssize_t)sizeof(calcMessage)){
            calcMessage cm{}; memcpy(&cm,buf,sizeof(cm)); uint16_t t=ntohs(cm.type); if(ntohs(cm.major_version)==1 && ntohs(cm.minor_version)==1 && (t==21||t==22)){
                bool wantText = (t==21);
                if(itAddr==clientToId.end()){
                    Task tk=makeTask(nextId++); tk.text=wantText; TaskRec rec{tk,ca,clen}; idTasks[tk.id]=rec; clientToId[key]=tk.id; 
                    if(wantText){ 
                    ...
                    DBG(cout<<"TASK id="<<tk.id<<" op="<<tk.op<<" v1="<<tk.v1<<" v2="<<tk.v2<<(wantText?" TEXT":" BIN")<<"\n");
                } else { 
                    TaskRec &rec=idTasks[itAddr->second]; Task &tk=rec.task; 
                    if(!tk.done){ 
                        if(tk.text){ 
                            string line=to_string(tk.id)+" "+(tk.op==1?"add":tk.op==2?"sub":tk.op==3?"mul":"div")+" "+to_string(tk.v1)+" "+to_string(tk.v2)+"\n"; 
                            sendto(sock,line.c_str(),line.size(),0,(sockaddr*)&ca,clen);
                ...
                if(t==2 && id>0){
                    ...
                    if(it!=idTasks.end()){
                        ...
                        if(!tk.done){
                            ...
                            DBG(cout<<"ANS id="<<id<<" ok="<<okAns<<(expired?" EXPIRED":"")<<"\n");
                            if(okAns) ++ok; else ++fail;
                        }
                        clientToId.erase(key);
                        // Remove task entry after answer (client lifecycle complete)
                        idTasks.erase(it);
                    }
                    continue;
                }
        ...
        // Text protocol handling
        {
            ...
            if(msg=="TEXT UDP 1.1"){
                if(itAddr==clientToId.end()){
                    Task tk=makeTask(nextId++); tk.text=true; TaskRec rec{tk,ca,clen}; idTasks[tk.id]=rec; clientToId[key]=tk.id;
                    string line=to_string(tk.id)+" "+(tk.op==1?"add":tk.op==2?"sub":tk.op==3?"mul":"div")+" "+to_string(tk.v1)+" "+to_string(tk.v2)+"\n";
                    sendto(sock,line.c_str(),line.size(),0,(sockaddr*)&ca,clen);
                    DBG(cout<<"TASK id="<<tk.id<<" text\n");
                } else { TaskRec &rec=idTasks[itAddr->second]; Task &tk=rec.task; if(!tk.done){ string line=to_string(tk.id)+" "+(tk.op==1?"add":tk.op==2?"sub":tk.op==3?"mul":"div")+" "+to_string(tk.v1)+" "+to_string(tk.v2)+"\n"; sendto(sock,line.c_str(),line.size(),0,(sockaddr*)&ca,clen);} }
                continue;
            }
            auto itA=(itAddr==clientToId.end()? clientToId.end(): itAddr);
            if(itA!=clientToId.end()){
                TaskRec &rec=idTasks[itA->second]; Task &tk=rec.task; size_t sp=msg.find(' '); if(sp!=string::npos){ bool pars=true; uint32_t rid=0; long long ans=0; try{ rid=stoul(msg.substr(0,sp)); ans=stoll(msg.substr(sp+1)); }catch(...){ pars=false; }
                    if(pars && rid==tk.id && !tk.done){
                        ...
                        DBG(cout<<"ANS id="<<tk.id<<" ok="<<okAns<<(expired?" EXPIRED":"")<<"\n");
                        clientToId.erase(key);
                        if(okAns) ++ok; else ++fail;
                        // Remove task entry after answer
                        idTasks.erase(tk.id);
                    }
                }
            }
        ...
        for(auto it=idTasks.begin(); it!=idTasks.end();){
            Task &tk=it->second.task; 
            if(!tk.done){ 
                auto age=chrono::duration_cast<chrono::seconds>(Clock::now()-tk.created).count(); 
                if(age>10){ // expire
                    // Remove client mapping using reconstructed key
                    TaskRec &rec = it->second; 
                    ClientKey ckey{rec.addr, rec.len};
                    clientToId.erase(ckey);
                    it=idTasks.erase(it); 
                    continue; 
                }
            }
