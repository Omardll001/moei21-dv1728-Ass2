...
int main(int argc,char*argv[]){
    if(argc<2){ cerr<<"Usage: "<<argv[0]<<" host:port [--text] [--quiet]\n"; return 1; }
    bool enableText=false, quiet=false; for(int i=2;i<argc;i++){ string f=argv[i]; if(f=="--text") enableText=true; else if(f=="--quiet") quiet=true; }
    ...
    unordered_map<ClientKey,Task,ClientHash> tasks; uint32_t nextId=1; size_t ok=0, fail=0, issued=0; auto start=Clock::now(); const int TARGET=100;
    ...
    while(g_run){
        ...
        if(sel>0){
            for(int s: sockets){ if(!FD_ISSET(s,&rf)) continue; for(int drain=0; drain<512; ++drain){ sockaddr_storage ca{}; socklen_t clen=sizeof(ca); unsigned char buf[128]; ssize_t n=recvfrom(s,buf,sizeof(buf),MSG_DONTWAIT,(sockaddr*)&ca,&clen); if(n<0){ if(errno==EAGAIN||errno==EWOULDBLOCK) break; perror("recvfrom"); break; } if(n==0) break; ClientKey key{ca,clen};
                ...
                if(n>=(ssize_t)24 && n<=(ssize_t)sizeof(calcProtocol)){ calcProtocol cp{}; memcpy(&cp,buf,std::min<size_t>(n,sizeof(cp))); if(ntohs(cp.major_version)!=1||ntohs(cp.minor_version)!=1) continue; uint32_t id=ntohl(cp.id); int32_t res=ntohl(cp.inResult); auto it=tasks.find(key); if(it==tasks.end()){ if(id==0){ Task t=makeTask(nextId++); tasks[key]=t; calcProtocol out{}; out.type=htons(1); out.major_version=htons(1); out.minor_version=htons(1); out.id=htonl(t.id); out.arith=htonl(t.op); out.inValue1=htonl(t.v1); out.inValue2=htonl(t.v2); sendto(s,&out,sizeof(out),0,(sockaddr*)&ca,clen); ++issued; } }
                            ...
                            else if(id==t.id){ bool okAns=(res==eval(t)); t.done=true; t.ok=okAns; if(okAns) ++ok; else ++fail; calcMessage m{}; m.type=htons(2); m.message=htonl(okAns?1:2); m.protocol=htons(17); m.major_version=htons(1); m.minor_version=htons(1); sendto(s,&m,sizeof(m),0,(sockaddr*)&ca,clen); if(!quiet && (ok+fail)%20==0){ auto ms=chrono::duration_cast<chrono::milliseconds>(now-start).count(); cout<<"PROGRESS ans="<<(ok+fail)<<" ok="<<ok<<" ms="<<ms<<"\n"; } if(ok>=TARGET && !quiet){ auto ms=chrono::duration_cast<chrono::milliseconds>(now-start).count(); cout<<"COMPLETE ok="<<ok<<" fail="<<fail<<" ms="<<ms<<"\n"; } } }
        ...
        auto now2=Clock::now(); for(auto &kv:tasks){ Task &t=kv.second; if(t.done) continue; auto ms=chrono::duration_cast<chrono::milliseconds>(now2 - t.lastSend).count(); if(ms < 120) continue; int target=(t.resend<2?120:(t.resend<5?240:480)); if(ms>=target){ calcProtocol out{}; out.type=htons(1); out.major_version=htons(1); out.minor_version=htons(1); out.id=htonl(t.id); out.arith=htonl(t.op); out.inValue1=htonl(t.v1); out.inValue2=htonl(t.v2); for(int s: sockets){ sendto(s,&out,sizeof(out),0,(sockaddr*)&kv.first.addr,kv.first.len); break; } t.lastSend=now2; ++t.resend; } }
        if(!quiet){ static auto lastPrint=start; if(chrono::duration_cast<chrono::milliseconds>(Clock::now()-lastPrint).count()>=1000){ size_t pending=0; for(auto &kv:tasks) if(!kv.second.done) ++pending; cout<<"DIAG tasks="<<issued<<" ok="<<ok<<" fail="<<fail<<" pend="<<pending<<"\n"; lastPrint=Clock::now(); } }
